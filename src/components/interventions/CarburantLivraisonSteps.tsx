...
export default function CarburantLivraisonSteps({ currentStep, formData, onNext, onPrevious, onSubmit }: CarburantLivraisonStepsProps) {
  ...
  // Auto-fill from saved context (only for step 1 - renseignement clients)
  useEffect(() => {
    if (currentStep === 1 && !contextApplied && clients.length > 0) {
      try {
        const saved = localStorage.getItem('last-intervention-context');
        if (saved) {
          const context = JSON.parse(saved);

          // Check if context is valid and matches type 'carburant-livraison'
          if (context.typePrestation === 'carburant-livraison' && Date.now() < context.expiresAt) {
